...
const ModeratorPanel = () => {
  ...
  const { authenticated, isModerator, user, logout } = useStore(
    (state) => state.authSlice
  );
  const { flaggedMessageCount, fetchFlaggedMessageCount } = useStore(
  ...
  const drawer = (
    <Box
      sx={{ bgcolor: "rgba(13, 37, 56, 0.95)", height: "100%", color: "white" }}
    >
