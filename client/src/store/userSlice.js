...
const userSlice = (set, get) => ({
    ...
    try {
      ...
      set(state => {
        if (page === 0) {
          ...
          state.userSlice.messages = [...state.userSlice.messages, ...(response.data.messages || [])];
          // Add any new unread messages to the count
          const existingUnreadCount = (state.userSlice.messages || [])
            .filter(msg => !msg.isRead)
            .length;
        ...
        state.userSlice.hasMoreMessages = response.data.pagination?.hasMore || false;
      });
  ...
  loadMoreMessages: async () => {
    ...
    try {
      ...
      set(state => { state.userSlice.messageLoading = false; });
      const message = error.response?.data?.error || 'Failed to load more messages';
      toast.error(message);
      return { success: false, message };
    }
  },
  ...
  sendMessage: async (messageData) => {
    ...
    try {
      ...
      const formattedData = {
        recipientUid: messageData.recipientId,
    ...
    try {
      ...
      set(state => {
        const updatedMessages = state.userSlice.messages.map(msg => 
          msg.messageId === messageId ? { ...msg, flagged: !currentlyFlagged } : msg
        );
        state.userSlice.messages = updatedMessages;
      });
      
      toast.success(currentlyFlagged ? 'Message unflagged' : 'Message flagged for review');
      return { success: true };
    } catch (error) {
      console.error('Error toggling message flag:', error);
      const message = error.response?.data?.error || 'Failed to toggle message flag';
      toast.error(message);
      return { success: false, message };
    }
  },
  ...
  searchUsers: async (query) => {
    ...
    try {
      ...
      set(state => {
        state.userSlice.users = response.data.users || [];
    ...
    try {
      ...
      set(state => {
        if (page === 0) {
          ...
          // Append messages for pagination
          state.userSlice.sentMessages = [...state.userSlice.sentMessages, ...(response.data.messages || [])];
        }
        state.userSlice.sentMessagePage = page;
        state.userSlice.sentMessageLoading = false;
        state.userSlice.hasMoreSentMessages = response.data.pagination?.hasMore || false;
      });
      ...
      set(state => { state.userSlice.sentMessageLoading = false; });
      const message = error.response?.data?.error || 'Failed to fetch sent messages';
      toast.error(message);
      return { success: false, message };
    }
  },
  ...
  loadMoreSentMessages: async () => {
    ...
    try {
      set(state => { state.userSlice.sentMessageLoading = true; });
      ...
      set(state => { state.userSlice.sentMessageLoading = false; });
      const message = error.response?.data?.error || 'Failed to load more sent messages';
      toast.error(message);
      return { success: false, message };
    }
  },
  ...
  loadMoreUsers: async (query = '') => {
    ...
    try {
      ...
      set(state => {
        state.userSlice.users = [...state.userSlice.users, ...response.data.users];
        state.userSlice.userLoading = false;
      ...
      set(state => { state.userSlice.userLoading = false; });
      const message = error.response?.data?.error || 'Failed to load more users';
      toast.error(message);
      return { success: false, message };
    }
  },
  ...
  markMessagesAsRead: async () => {
    ...
    try {
      ...
      return { success: false, message: error.response?.data?.error || 'Failed to mark messages as read' };
    }
  },
  ...
  checkNewMessages: async () => {
    ...
    try {
      ...
      return { success: false, message: error.response?.data?.error || 'Failed to check for new messages' };
    }
  },
  ...
  fetchFlaggedMessageCount: async () => {
    ...
    try {
      ...
      return { success: false, message: error.response?.data?.error || 'Failed to fetch flagged message count' };
    }
  },
