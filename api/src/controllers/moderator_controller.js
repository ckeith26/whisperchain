...
export const getFlaggedMessages = async (req, res) => {
  try {
    ...
    if (!moderator) {
      return res.status(403).json({ error: 'You do not have permission to access flagged messages' });
    }
    ...
    const enhancedMessages = await Promise.all(
      flaggedMessages.map(async (msg) => {
        ...
        if (msg.senderToken) {
          const authToken = await AuthTokenModel.findOne({ token: msg.senderToken });
          if (authToken) {
    ...
    return res.json({
      ...
      pagination: {
        ...
        limit,
        totalCount,
        hasMore: skip + flaggedMessages.length < totalCount,
      },
    });
  } catch (error) {
...
export const getFlaggedMessageCount = async (req, res) => {
  try {
    ...
    if (!moderator) {
      return res.status(403).json({ error: 'You do not have permission to access flagged message count' });
    }
...
export const getTokenInfo = async (req, res) => {
  try {
    ...
    if (!moderator) {
      return res.status(403).json({ error: 'You do not have permission to access token information' });
    }
...
export const moderateMessage = async (req, res) => {
  try {
    ...
    if (!moderator) {
      return res.status(403).json({ error: 'You do not have permission to moderate messages' });
    }
