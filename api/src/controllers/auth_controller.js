...
export const login = async (req, res) => {
  try {
    ...
    if (!user.password) {
      ...
      return res
        .status(500)
        .json({ error: "User password not properly stored" });
    }
    ...
    try {
      ...
      return res
        .status(500)
        .json({ error: "Error comparing passwords: " + compareError.message });
    }
...
export const generateKeyPair = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: error.message || "Failed to store public key" });
  }
};
...
export const searchUsers = async (req, res) => {
  try {
    ...
    let userQuery = {
      ...
      publicKey: { $exists: true, $ne: null }, // Only include users with public keys
    };

    // Find users with public keys
    const users = await UserModel.find(userQuery)
      .select("uid publicKey")
      .sort({ _id: -1 })
      .skip(skip)
      .limit(limit);
